...
pkl_file = open('../pickle_files/Lat_Disk_Height.pkl', 'rb')
...
mcp_save = ModelCheckpoint('Lat_Disk_Height.hdf5', save_best_only=True, monitor='val_loss', mode='min')
reduce_lr_loss = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=7, verbose=1, epsilon=1e-4, mode='min')

model.fit(X_train, Y_train, validation_data = (X_test, Y_test), batch_size=1, callbacks=[earlyStopping, mcp_save, reduce_lr_loss], epochs=30)
...
test_result = (test_result > 0.1).astype(np.uint8)
...
for i in range(X_test.shape[0]):
    ...
    plt.savefig(fname = os.path.join("../samples/Lat_Disk_Height", str(i)+".png"))
    plt.show()
