...
pkl_file = open('../pickle_files/Lat_Posterior_Vertebral_Line.pkl', 'rb')
...
mcp_save = ModelCheckpoint('Lat_Posterior_Vertebral_Line.hdf5', save_best_only=True, monitor='val_loss', mode='min')
...
test_result = (test_result > 0.02).astype(np.uint8)
...
for i in range(X_test.shape[0]):
    ...
    plt.savefig(fname = os.path.join("../samples/Lat_Posterior_Vertebral_Line", str(i)+".png"))
    plt.show()
